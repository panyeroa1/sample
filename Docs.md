...
- Views are lazily loaded via `CenterPanel` and keyed by `ActiveView`.
...
- **Chatbot (`components/ChatbotView.tsx`)**: multimodal chat with history persistence, optional web/maps grounding, “Pro” thinking mode, fast mode, developer mode that returns self-contained HTML and renders in the right-hand simulator, image upload, mic-to-text via Gemini, telemetry (tokens/energy/wps), and grounding source display (Synced to Supabase + IDB cache).
- **TTS Studio (`components/TTSStudioView.tsx`)**: Gemini TTS generator with on-page audio preview/download; generations are uploaded to Supabase Storage and logged to the `tts_generations` table with offline caching in IndexedDB.
...
- **CRM (`components/CrmView.tsx` + `CrmDetailView`, `BookingModal`)**: browse/search bookings, view details with notes/timeline, create/edit/delete via modal; data updates broadcast through `crmService` subscriptions and persisted to Supabase with IndexedDB cache.
...
- Feedback: quick modal submits text to IDB and attempts Supabase sync; agent feedback submissions use the same path.
...
- CRM bookings (`crm_bookings` table) — fetched and upserted through `dataService` + `supabaseService`, cached in IndexedDB, and mirrored into `crmService` for subscriptions.
...
- For any new session-only features, add Supabase upserts in `dataService` and call them from the views; keep IndexedDB as the offline cache.
