...
import { AgentIcon, UserIcon, GlobeIcon, MicIcon, PhoneIcon, ClipboardEditIcon, SaveIcon, CheckCircleIcon, DatabaseIcon, CpuIcon, MailIcon, CalendarIcon, ChevronLeftIcon } from './icons';
import { AUDIO_ASSETS, CRM_TOOLS } from '../constants';
...
type ActiveMainTab = 'transcription' | 'crm' | 'tools';
...
const WebDemoView: React.FC<WebDemoViewProps> = ({ template, onEndDemo }) => {
    ...
    useEffect(() => {
        ...
        bgNoiseAudioRef.current.volume = 0.1;
            ...
            try {
                ...
                if (voiceId) {
                    const agentForSession: Agent = {
                        ...
                        voice: voiceId, thinkingMode: false, avatarUrl: null, tools: CRM_TOOLS,
                    };
    ...
    const startAylaSequence = useCallback(async () => {
        ...
        try {
            ...
            audioRef.current.onended = async () => {
                try {
                    await startSession(agent, CRM_TOOLS);
                } catch (error) {
    ...
    const handleDialpadPress = useCallback((key: string) => {
        if (callState === 'awaitingInput') {
           switch(key) {
               ...
               case '2': // Turkish - Ayla is multilingual
                   startAylaSequence();
                   break;
    ...
    const handleStartCall = () => {
        ...
        const ivrAudioPromise = dataService.generateTtsWithGemini(ivrPrompt);
        ...
        return (
             <div className="w-full h-full flex flex-col bg-gray-900 text-white p-4">
                    ...
                    {callState === 'idle' && (
                        <>
                            ...
                            <Dialpad onKeyPress={handleDialerInput} keys={IVR_DIALPAD_KEYS} />
                             <button onClick={() => handleDialerInput('backspace')} className="text-lg font-semibold">
                                Delete
                            </button>
                        </>
                    )}
                ...
                <div className="mt-auto flex items-center justify-center">
                     {callState === 'idle' ? (
                        <button onClick={handleStartCall} className="w-20 h-20 rounded-full bg-eburon-ok flex items-center justify-center group transition-transform hover:scale-105">
